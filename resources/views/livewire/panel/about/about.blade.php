...
>
    ...
    <h3 class="mb-3">{{ __('Manage About Details') }}</h3>
    ...
    <div class="p-3 rounded-3 bg-white shadow-md mt-2" x-cloak x-show="add" x-transition>
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="code-tab" data-bs-toggle="tab" data-bs-target="#code"
                    type="button" role="tab" aria-controls="code" aria-selected="true">{{__('Add About')}}</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="preview-tab" data-bs-toggle="tab" data-bs-target="#preview"
                    type="button" role="tab" aria-controls="preview" aria-selected="false">{{__('Preview Demo')}}</button>
            </li>
        </ul>
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane active" id="code" role="tabpanel" aria-labelledby="code-tab">
                <x-forms.textarea>
                    <x-slot:for>{{ 'about' }}</x-slot:for>
                    <x-slot:placeholder>{{ 'About OsamaGenius Write / Paste' }}</x-slot:placeholder>
    ...
    <div class="p-3 rounded-3 bg-white shadow-md mt-2" x-cloak x-show="show" x-transition>
        ...
        <div class="text-end">
            <div class="btn-group">
                <button class="btn btn-outline-success"><i class="fas fa-edit me-1"></i>{{__('Edit')}}</button>
                <button class="btn btn-outline-danger"><i class="fas fa-trash me-1"></i>{{__('Delete')}}</button>
            </div>
        </div> --}}
                ...
                <thead class="table-light text-center">
                    ...
                    <tbody class="table-group-divider">
                        @if (count($aboutDetails) > 0)
                            @foreach ($aboutDetails as $i => $item)
                                <tr class="border-bottom">
                                    <td scope="row">{{$i+=1}}</td>
